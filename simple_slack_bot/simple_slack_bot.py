...
class SimpleSlackBot:
    ...
    def helper_get_user_identity(self):
        a = self._slacker.users.identity()
        print(f'!!!{a}')

    def helper_ser_id_to_user_display_name(self, user_id):
        """
        유저 아이디로 유저 보이는 이름 가져오는 함수
        ...
        for user in users_list.body["members"]:
            if user["id"] == user_id:
                print('AAA'+user["profile"]["display_name"])
